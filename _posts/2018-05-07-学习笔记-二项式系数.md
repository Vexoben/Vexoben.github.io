...
$$
\dbinom{n}{k}=C_n^k=\begin{cases}
   \ \frac{n(n-1)...(n-k+1)}{k!} & k>=1 \\
   \ 1 & k=0 \\
   \ 0 & k<=-1 \\  
   \end{cases} $$
...
设$$ \alpha $$是实数。对于所有满足$$ 0<=\left|x\right|<\left|y\right|$$ 的$$x$$和$$y$$，有
...
设$$z=x/y$$，则对于所有$$\left|z\right|<1$$，有
...
7、（牛顿二项式定理）对整数$$\alpha>=0$$或$$\left|x\right|<\left|y\right|$$有
