...
接着考虑如何求出$$lim_i$$。我们可以先无视$$l,r$$的限制，求出$$T[1..i]$$在$$S$$上能匹配的最长后缀。这直接像AC自动机一样跳，只是用$$parent$$树边代替$$fail$$边。假设匹配出来的最长字符串是$$s$$，我们不断删去$$s$$的第一个字符，使得它在$$S[l..r]$$上出现即可，也即是对当前节点判断$$right$$集合是否包含$$[l+s.lengt-1,r]$$中的数，线段树合并可以轻易地解决这个问题。
