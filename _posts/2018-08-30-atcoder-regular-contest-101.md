...
显然对答案会产生影响的是历史向左走最大值和向右走最大值 $$ （x, y） $$ ， 且每次可以让 $$ x $$ 或 $$ y $$ 加一。
...
我们可以枚举最后一个由向上转至向右的拐角在哪里，设转角在 $$ (x_{i}, y_{i}) $$ 的方案数。

那么 $$ dp[i] = (\sum _ {j = 1} ^ {i - 1} [y_{j} < y_{i}] dp[j]) + 1 $$
